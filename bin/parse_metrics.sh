...
snps_2=$(grep -P '\tPASS\t' ${ID}_filtered_snps.vcf | wc -l)
avg_coverage=$(awk '{sum+=$3} END { print sum/NR}' ${ID}_depth_out.txt)

echo "ID,# reads,aligned reads,% aligned,aligned bases,read length,% paired, %dup, mean insert size,# snps, # snps filtered, average coverage" 
echo "$ID,$ALIGNMENT_METRICS,$DUP_PCT,$MEAN_INSERT_SIZE,$snps_1,$snps_2,$avg_coverage"
